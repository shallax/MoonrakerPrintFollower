...
class FollowerTransportMixin:
    ...
    def __init__(self, application) -> None:
        # Transport-owned transient state is initialised before the runtime so
        # synchronous startup callbacks can safely inspect it. Qt-owned probe
        # state is created after the runtime has initialised QObject.
        self._pending_status_purpose: Optional[str] = None
        self._pending_status_generation = 0
        self._follower_probe_transport = None
    ...
    def _issue_status_request(self, base_url: str, api_key: str, purpose: str) -> None:
        ...
        started = self._follower_probe_transport.send_json(
            ...
            lambda payload, error, p=purpose, g=generation: self._on_probe_status(
                payload, error, p, g
            ),
            replace=True,
    ...
    def _ensure_remote_metadata(self, filename: str, fallback_size: int = 0) -> None:
        ...
        if (
            ...
            and self._remote_file_service.metadata_job_key == job_key
        ):
            return
        ...
        generation = self._cura_lifecycle_bridge.generation
        started = self._client.transport.send_json(
            "follower",
            "metadata",
            "GET",
            metadata_endpoint(base_url, filename),
            lambda payload, error, f=filename, sz=int(fallback_size or 0), g=generation, j=job_key:
                self._handle_metadata_payload(payload, error, f, sz, g, j),
            replace=True,
    ...
    def _handle_metadata_payload(
        ...
        if error:
            ...
            self._remote_file_service.set_identity(
                RemoteFileIdentity(filename, int(fallback_size or 0)), current_job
            )
        ...
        try:
            ...
            if current_job is not None and not identity.matches_job(
                ...
                return
            self._remote_file_service.set_identity(identity, current_job)
            if (
                self._gcode_index_service.data is not None
                and self._gcode_index_service.filename == filename
                and (identity.uuid or identity.modified > 0)
            ):
                self._persist_index_async(identity, self._gcode_index_service.data)
            ...
            Logger.log(
                "w",
                "Moonraker Print Follower metadata lookup failed for %s: %s",
                filename,
                exc,
            )
    ...
    def _begin_gcode_download(self, filename: str) -> bool:
        ...
        if self._file_reply is not None and self._file_reply.isRunning():
            if (
                ...
                and self._file_reply_job_key == current_job
            ):
                return True
        ...
        try:
            ...
            base_name = (
                os.path.basename(filename.replace("\\", "/")) or "moonraker.gcode"
            )
            ...
            Logger.logException(
                "w",
                "Moonraker Print Follower could not create download target: %s",
                exc,
            )
            return False

        request = self._client.transport.request(
            download_endpoint(base_url, filename), timeout_ms=30000
        )
        request.setRawHeader(b"Accept", b"application/octet-stream")
        self._file_reply_filename = filename
        self._file_reply_job_key = current_job
        ...
        reply.finished.connect(
            lambda r=reply, f=filename, g=reply_generation, j=reply_job_key:
                self._handle_gcode_reply(r, f, g, j)
        )
        return True
    ...
    def _send_scheduled_pause(self, target_layer: int, current_layer: int) -> None:
        ...
        if not self._url_is_usable(base_url):
            self._set_status(
                f"Could not PAUSE after layer {target_layer + 1}: Moonraker URL unavailable"
            )
            return
        ...
        self._client.track_command(
            self.SCHEDULED_PAUSE_COMMAND, {"paused"}, timeout_s=10.0
        )
        ...
        if not started:
            ...
            self._set_status(
                f"Could not PAUSE after layer {target_layer + 1}: request already in flight"
            )
            return
        Logger.log(
            "i",
    ...
    def _on_scheduled_pause_http_finished(
        ...
        if (
            request_generation != self._scheduled_pause_request_generation
            or lifecycle_generation != self._cura_lifecycle_bridge.generation
            or job_key != self._remote_job_service.key
        ):
            return
        ...
        if error:
            ...
            if target_layer is not None:
                self._set_status(
                    f"PAUSE after layer {target_layer + 1} failed: {error}"
                )
            return
        self._client.accept_command(self.SCHEDULED_PAUSE_COMMAND)
        if target_layer is not None:
            observed_human = (
                observed_layer + 1 if observed_layer is not None else "?"
            )
            self._set_status(
    ...
    def _on_follower_command_changed(self, event: Any) -> None:
        if (
            ...
            or str(event.get("name") or "") != self.SCHEDULED_PAUSE_COMMAND
        ):
            return
