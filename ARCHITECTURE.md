...
`MoonrakerPrintFollower.py` is only the public Cura extension facade. `FollowerCoordinator.py` composes focused domain services. `FollowerRuntime.py` contains only the remaining Cura-facing runtime mechanics that have not moved into a domain service or transport adapter.

Each mutable domain has one authoritative owner:

- `RemoteJobService.py` owns remote print-run identity and restart detection.
- `RemoteFileService.py` owns remote-file identity plus downloaded G-code cache state.
- `GCodeIndexService.py` owns active index, build and layer-hydration state. Pure G-code parsing/index algorithms and data structures remain in `GCodeIndex.py`.
...
There are intentionally no parallel legacy service classes for these domains. `PauseScheduler.py`, `PreviewController.py`, `PrintTracker.py`, `GCodeRepository.py`, `FollowerStateBridge.py` and `FollowerSession.py` were transitional duplicates and are removed.

The remaining split is structural rather than behavioural: established Cura-facing behaviour is preserved while ownership is moved into focused services. Runtime code must not contain shadow implementations for service-owned transport, job, file, Preview, pause-scheduling or lifecycle responsibilities.
...
- `GCodeIndex.py` contains pure indexing/timing/motion algorithms and data structures covered by file fixtures.
...
- Large G-code streaming is a specialised follower transport operation using the shared active-printer HTTP connection pool.
...
Pure policy/state has deterministic unit coverage, including a scripted fake Moonraker harness for status progression, command acknowledgement, request coalescing, scheduled-pause precision polling, long-print state progression and print restart cases. Source-contract tests enforce one authoritative service per extracted state domain so duplicate replacement classes cannot silently accumulate again. Cura/QML source contracts and G-code fixtures remain regression guards around the runtime boundary.
